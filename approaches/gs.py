...
class Appr(object):
    ...
    def train(self, t, xtrain, ytrain, xvalid, yvalid, data, input_size, taskcla):
            ...
            for name, param in self.model.named_parameters():
                ...
                if "conv1" not in name:
                    if "conv" in name:  # convolution layer
                        ...
                        temp = torch.ones_like(param)
                        temp = temp.reshape(
                            (temp.size(0), self.omega[prekey].size(0), -1)
                        )
                        temp[:, self.omega[prekey] == 0] = 0
                        temp[self.omega[key] == 0] = 1
        ...
        torch.save(
            ...
            "./trained_model/" + self.log_name + "_task_{}.pt".format(t),
        )
        ...
        for (name, dummy_layer), (_, layer) in zip(
            dummy.named_children(), self.model.named_children()
        ):
            with torch.no_grad():
                if isinstance(layer, nn.Linear) or isinstance(layer, nn.Conv2d):
                    ...
                    indice = np.random.choice(
                        range(zero_cnt), int(zero_cnt * (1 - args.rho)), replace=False
                    )
                    ...
                    layer.weight.data = (
                        1 - mask
                    ) * layer.weight.data + mask * dummy_layer.weight.data
                    mask = mask.squeeze()
                ...
                if isinstance(layer, nn.ModuleList):
                    ...
                    if "omniglot" in args.experiment:
                        weight = weight.view(
                            weight.shape[0], self.omega[pre_name].shape[0], -1
                        )
    ...
    def proxy_grad_descent(self, t, lr):
        with torch.no_grad():
            for (name, module), (_, module_old) in zip(
                self.model.named_children(), self.model_old.named_children()
            ):
                ...
                if len(weight.size()) > 2:
                    sparse_weight = weight.data * coeff.unsqueeze(-1).unsqueeze(
                        -1
                    ).unsqueeze(-1)
                else:
                ...
                if t > 0:
                    ...
                    aux = F.threshold(
                        norm - self.omega[key] * self.lamb * lr, 0, 0, False
                    )
                    ...
                    if len(weight.size()) > 2:
                        penalty_weight = (
                            coeff_alpha.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                            * weight.data
                            + coeff_beta.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                            * weight_old.data
                        )
                    else:
                        penalty_weight = (
                            coeff_alpha.unsqueeze(-1) * weight.data
                            + coeff_beta.unsqueeze(-1) * weight_old.data
                        )
                    penalty_bias = coeff_alpha * bias.data + coeff_beta * bias_old.data

                diff_weight = sparse_weight + penalty_weight
                diff_bias = sparse_bias + penalty_bias
